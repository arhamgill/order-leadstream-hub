...
function getPackageLabel(cart: CartItem[], category: string, type: string) {
  return cart
    ...
    .map((i) => `${i.quantity} Pack — ${money(i.price)}`)
    .join(', ');
}
...
function FieldError({ msg }: { msg?: string }) {
  if (!msg) return null;
  return (
    <p role="alert" data-testid="field-error" className="mt-1.5 text-[11px] font-medium text-[#f4a8a8]">
      {msg}
    </p>
  );
}
...
function StatesMultiSelect({ value, onChange, error, id }: {
  ...
  const label =
    value.length === 0 ? 'Select states…'
    : value.length === US_STATES.length ? 'All states selected'
    : value.slice(0, 5).join(', ') + (value.length > 5 ? ` +${value.length - 5} more` : '');

  return (
    <div className="relative" ref={ref}>
      <button
        type="button" id={id} data-testid={`${id}-trigger`}
        onClick={() => setOpen((o) => !o)}
        ...
        {open && (
          ...
          >
            ...
            <div role="listbox" aria-multiselectable="true" className="scrollbar-subtle grid max-h-52 grid-cols-5 gap-px overflow-y-auto p-2">
              {US_STATES.map((state) => (
                <button
                  key={state} type="button" role="option" aria-selected={value.includes(state)}
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1.5">
        {DAYS.map((day) => (
          <button key={day} type="button" data-testid={`${id}-day-${day.toLowerCase()}`}
            onClick={() => toggle(day)}
      ...
      <input id={`${id}-hours`} type="text" data-testid={`${id}-hours`}
...
function ProductSectionCard({ title, badge, override, children, overrideToggle, overrideFields }: {
  ...
  overrideToggle: React.ReactNode;
  overrideFields: React.ReactNode;
}) {
  return (
      ...
      <div className="px-5 py-4 sm:px-6">
          ...
          {cart.map((item) => (
            <div key={item.lineId} className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-2 min-w-0">
                ...
                <span className="text-[11px] font-semibold leading-5 text-white">
                  {item.quantity} {item.category} {item.type}
                </span>
              </div>
              <span className="font-display text-[11px] font-semibold text-[#d8eaff] shrink-0">
        ...
        <div className="mt-4 space-y-2 border-t border-[#1d3558] pt-4 text-[11px]">
          <div className="flex justify-between text-[#7a95ba]">
            <span>Subtotal</span>
            <span className="font-display font-semibold text-white" data-testid="text-summary-subtotal">{money(subtotal)}</span>
          </div>
          {savings > 0 && (
            <div className="flex justify-between text-[#62d8f0]">
              <span>Estimated Savings</span>
              <span className="font-display font-semibold" data-testid="text-summary-savings">{money(savings)}</span>
            </div>
          )}
          <div className="flex items-center justify-between text-[#7a95ba]">
            ...
            <span className="text-right font-semibold text-[#8fa6c4]">Invoiced via rep</span>
          </div>
          <div className="flex justify-between border-t border-[#1d3558] pt-2.5 text-[12px]">
            ...
            <span className="font-display font-bold text-white" data-testid="text-summary-total">{money(subtotal)}</span>
          </div>
        </div>
...
function SuccessScreen({ cart, onBackToShop }: { cart: CartItem[]; onBackToShop: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}
      ...
      <h2 className="font-display text-3xl font-semibold tracking-[-0.04em] text-white sm:text-4xl">You're all set.</h2>
      <p className="mt-4 max-w-[420px] text-sm leading-7 text-[#8fa6c4]">
        Your order has been submitted and is being reviewed by the LeadStream Hub team. A representative will reach out shortly to finalize payment and delivery.
      </p>
      <div className="mt-8 w-full max-w-[420px] rounded-[16px] border border-[#243e62] bg-[#0d1b2e] p-5 text-left">
          ...
          {cart.map((item) => (
            <div key={item.lineId} className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-xs text-[#8fa6c4] truncate">
                  {item.quantity} {item.category} {item.type}
                </span>
              </div>
              <span className="font-display text-xs font-semibold text-white shrink-0">{money(item.price * item.quantity)}</span>
            </div>
          ))}
        </div>
        <div className="mt-4 flex items-center justify-between border-t border-[#1d3558] pt-4">
...
type FormValues = {
  ...
  // Default settings (text inputs managed by RHF; arrays managed by state)
  default_startDate: string;
  default_availHours: string;
  default_agencyMention: string;
  // Override toggles
  fe_cb_override: boolean;
  fe_lt_override: boolean;
  fe_pc_override: boolean;
  mc_cb_override: boolean;
  mc_lt_override: boolean;
  // Product-specific (always required when product in cart)
...
export default function CheckoutPage({ cart, onBack }: {
  ...
  const onSubmit = (_data: FormValues) => {
    ...
    // Per-product override validation
    if (hasFeCb && feCbOverride) {
      if (feCbOvStates.length === 0) errs.fe_cb_ov_states = 'Please select at least one state.';
    }
    if (hasFeLt && feLtOverride) {
      if (feLtOvStates.length === 0) errs.fe_lt_ov_states = 'Please select at least one state.';
      if (feLtOvDays.length === 0)   errs.fe_lt_ov_days   = 'Please select at least one day.';
    }
    if (hasFePc && fePcOverride) {
      if (fePcOvStates.length === 0) errs.fe_pc_ov_states = 'Please select at least one state.';
      if (fePcOvDays.length === 0)   errs.fe_pc_ov_days   = 'Please select at least one day.';
    }
    if (hasMcCb && mcCbOverride) {
      if (mcCbOvStates.length === 0) errs.mc_cb_ov_states = 'Please select at least one state.';
    }
    if (hasMcLt && mcLtOverride) {
      if (mcLtOvStates.length === 0) errs.mc_lt_ov_states = 'Please select at least one state.';
      if (mcLtOvDays.length === 0)   errs.mc_lt_ov_days   = 'Please select at least one day.';
    }
  ...
  const overrideToggle = (name: keyof FormValues, label: string) => (
    <label className="flex cursor-pointer items-center gap-3" data-testid={`toggle-${name}`}>
      ...
      <span className="text-xs font-semibold text-[#8fa6c4]">{label}</span>
    </label>
  );
    ...
    <motion.div className="portal-shell min-h-[100dvh]"
      ...
      <main className="mx-auto w-full max-w-[1200px] px-5 pb-28 sm:px-8 lg:px-10">
        <div className="mb-8 pt-10">
          ...
          <p className="mt-2 text-sm text-[#7a95ba]">Fill in your details below. Our team will review and follow up to finalize your order.</p>
        </div>

        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
          <form onSubmit={handleSubmit(onSubmit)} noValidate data-testid="form-checkout" className="space-y-5">
              ...
              <Row2>
                <div>
                  ...
                  <input id="phone" type="tel" data-testid="input-phone" placeholder="(555) 000-0000"
                    ...
                    {...register('phone', { required: 'Phone is required.', pattern: { value: /^[\d\s\-().+]{7,20}$/, message: 'Invalid phone.' } })} />
            ...
            <DefaultSettingsCard>
              ...
              <div>
                ...
                <AvailabilityPicker
                  id="default_avail"
                  days={defaultAvailDays} onDaysChange={setDefaultAvailDays}
                  hours={''} onHoursChange={() => {}}
                  error={extraErrors.default_availDays}
                />
            ...
            {hasFeCb && (
              <ProductSectionCard
                title="Final Expense — Callback Leads" badge="Product Details"
                override={!!feCbOverride}
                overrideToggle={overrideToggle('fe_cb_override', 'Override Default Settings for this product')}
                overrideFields={<>
                  <div>
                    <Label htmlFor="fe_cb_ov_states" required>Licensed States</Label>
                    <StatesMultiSelect id="fe_cb_ov_states" value={feCbOvStates} onChange={setFeCbOvStates} error={extraErrors.fe_cb_ov_states} />
                    <FieldError msg={extraErrors.fe_cb_ov_states} />
                  </div>
                  <div>
                    <Label htmlFor="fe_cb_ov_startDate" required>Preferred Start Date</Label>
                    {dateInput('fe_cb_ov_startDate', 'input-fe-cb-ov-start-date', 'fe_cb_ov_startDate', true)}
                  </div>
                  <div>
                    <Label htmlFor="fe_cb_ov_agencyMention" required>Agency Name to Mention</Label>
                    <input id="fe_cb_ov_agencyMention" data-testid="input-fe-cb-ov-agency"
                      placeholder="Override agency name for this product"
                      className={errors.fe_cb_ov_agencyMention ? errorInputClass : inputClass}
                      {...register('fe_cb_ov_agencyMention', { required: feCbOverride ? 'Agency name is required.' : false })} />
                    <FieldError msg={errors.fe_cb_ov_agencyMention?.message} />
                  </div>
                </>}
              >
                <div>
                  <Label htmlFor="fe_cb_package">Package</Label>
                  <div id="fe_cb_package" data-testid="display-fe-cb-package" className="rounded-[10px] border border-[#243e62] bg-[#080f1c] px-3.5 py-2.5 text-sm text-[#7a95ba]">
                    {getPackageLabel(cart, 'Final Expense', 'Callback Leads')}
                  </div>
                </div>
                <div>
                  <Label htmlFor="fe_cb_deliveryDate" required>Preferred Delivery Date</Label>
                  {dateInput('fe_cb_deliveryDate', 'input-fe-cb-delivery-date', 'fe_cb_deliveryDate', true)}
                </div>
                <div>
                  <Label htmlFor="fe_cb_format" required>Delivery Format</Label>
                  <RadioGroup name="fe_cb_format" options={['CSV', 'PDF']}
                    register={register as (n: string, o?: object) => object}
                    error={errors.fe_cb_format?.message} />
                </div>
              </ProductSectionCard>
            )}
            ...
            {hasFeLt && (
              <ProductSectionCard
                title="Final Expense — Live Transfers" badge="Product Details"
                override={!!feLtOverride}
                overrideToggle={overrideToggle('fe_lt_override', 'Override Default Settings for this product')}
                overrideFields={<>
                  <div>
                    <Label htmlFor="fe_lt_ov_states" required>Licensed States</Label>
                    <StatesMultiSelect id="fe_lt_ov_states" value={feLtOvStates} onChange={setFeLtOvStates} error={extraErrors.fe_lt_ov_states} />
                    <FieldError msg={extraErrors.fe_lt_ov_states} />
                  </div>
                  <div>
                    <Label htmlFor="fe_lt_ov_startDate" required>Preferred Start Date</Label>
                    {dateInput('fe_lt_ov_startDate', 'input-fe-lt-ov-start-date', 'fe_lt_ov_startDate', true)}
                  </div>
                  <div>
                    <Label htmlFor="fe_lt_ov_avail" required>Availability</Label>
                    <AvailabilityPicker id="fe_lt_ov_avail" days={feLtOvDays} onDaysChange={setFeLtOvDays}
                      hours={''} onHoursChange={() => {}} error={extraErrors.fe_lt_ov_days} />
                    <FieldError msg={extraErrors.fe_lt_ov_days} />
                  </div>
                </>}
              >
                <div>
                  <Label htmlFor="fe_lt_package">Package</Label>
                  <div id="fe_lt_package" data-testid="display-fe-lt-package" className="rounded-[10px] border border-[#243e62] bg-[#080f1c] px-3.5 py-2.5 text-sm text-[#7a95ba]">
                ...
                <Row2>
                  ...
                  <div>
                    ...
                    <input id="fe_lt_callsPerDay" type="number" min="1" data-testid="input-fe-lt-calls-per-day" placeholder="e.g. 10"
                      className={errors.fe_lt_callsPerDay ? errorInputClass : inputClass}
                      {...register('fe_lt_callsPerDay', { required: 'Calls per day is required.', min: { value: 1, message: 'Min 1.' } })} />
            ...
            {hasFePc && (
              <ProductSectionCard
                title="Final Expense — Pre Closed Applications" badge="Product Details"
                override={!!fePcOverride}
                overrideToggle={overrideToggle('fe_pc_override', 'Override Default Settings for this product')}
                overrideFields={<>
                  <div>
                    <Label htmlFor="fe_pc_ov_states" required>Licensed States</Label>
                    <StatesMultiSelect id="fe_pc_ov_states" value={fePcOvStates} onChange={setFePcOvStates} error={extraErrors.fe_pc_ov_states} />
                    <FieldError msg={extraErrors.fe_pc_ov_states} />
                  </div>
                  <div>
                    <Label htmlFor="fe_pc_ov_startDate" required>Preferred Start Date</Label>
                    {dateInput('fe_pc_ov_startDate', 'input-fe-pc-ov-start-date', 'fe_pc_ov_startDate', true)}
                  </div>
                  <div>
                    <Label htmlFor="fe_pc_ov_avail" required>Availability</Label>
                    <AvailabilityPicker id="fe_pc_ov_avail" days={fePcOvDays} onDaysChange={setFePcOvDays}
                      hours={''} onHoursChange={() => {}} error={extraErrors.fe_pc_ov_days} />
                    <FieldError msg={extraErrors.fe_pc_ov_days} />
                  </div>
                </>}
              >
                <div>
                  <Label htmlFor="fe_pc_package">Package</Label>
                  <div id="fe_pc_package" data-testid="display-fe-pc-package" className="rounded-[10px] border border-[#243e62] bg-[#080f1c] px-3.5 py-2.5 text-sm text-[#7a95ba]">
            ...
            {hasMcCb && (
              <ProductSectionCard
                title="Medicare — Callback Leads" badge="Product Details"
                override={!!mcCbOverride}
                overrideToggle={overrideToggle('mc_cb_override', 'Override Default Settings for this product')}
                overrideFields={<>
                  <div>
                    <Label htmlFor="mc_cb_ov_states" required>Licensed States</Label>
                    <StatesMultiSelect id="mc_cb_ov_states" value={mcCbOvStates} onChange={setMcCbOvStates} error={extraErrors.mc_cb_ov_states} />
                    <FieldError msg={extraErrors.mc_cb_ov_states} />
                  </div>
                  <div>
                    <Label htmlFor="mc_cb_ov_startDate" required>Preferred Start Date</Label>
                    {dateInput('mc_cb_ov_startDate', 'input-mc-cb-ov-start-date', 'mc_cb_ov_startDate', true)}
                  </div>
                  <div>
                    <Label htmlFor="mc_cb_ov_agencyMention" required>Agency Name to Mention</Label>
                    <input id="mc_cb_ov_agencyMention" data-testid="input-mc-cb-ov-agency"
                      placeholder="Override agency name for this product"
                      className={errors.mc_cb_ov_agencyMention ? errorInputClass : inputClass}
                      {...register('mc_cb_ov_agencyMention', { required: mcCbOverride ? 'Agency name is required.' : false })} />
                    <FieldError msg={errors.mc_cb_ov_agencyMention?.message} />
                  </div>
                </>}
              >
                <div>
                  <Label htmlFor="mc_cb_package">Package</Label>
                  <div id="mc_cb_package" data-testid="display-mc-cb-package" className="rounded-[10px] border border-[#243e62] bg-[#080f1c] px-3.5 py-2.5 text-sm text-[#7a95ba]">
                    {getPackageLabel(cart, 'Medicare', 'Callback Leads')}
                  </div>
                </div>
                <div>
                  <Label htmlFor="mc_cb_leadType" required>Lead Type</Label>
                  <RadioGroup name="mc_cb_leadType" options={['Medicare Supplement','Medicare Advantage','Both']}
                    register={register as (n: string, o?: object) => object}
                    error={errors.mc_cb_leadType?.message} />
                </div>
                <div>
                  <Label htmlFor="mc_cb_deliveryDate" required>Preferred Delivery Date</Label>
                  {dateInput('mc_cb_deliveryDate', 'input-mc-cb-delivery-date', 'mc_cb_deliveryDate', true)}
                </div>
                <div>
                  <Label htmlFor="mc_cb_format" required>Delivery Format</Label>
                  <RadioGroup name="mc_cb_format" options={['CSV','PDF']}
                    register={register as (n: string, o?: object) => object}
                    error={errors.mc_cb_format?.message} />
                </div>
              </ProductSectionCard>
            )}
            ...
            {hasMcLt && (
              <ProductSectionCard
                title="Medicare — Live Transfers" badge="Product Details"
                override={!!mcLtOverride}
                overrideToggle={overrideToggle('mc_lt_override', 'Override Default Settings for this product')}
                overrideFields={<>
                  <div>
                    <Label htmlFor="mc_lt_ov_states" required>Licensed States</Label>
                    <StatesMultiSelect id="mc_lt_ov_states" value={mcLtOvStates} onChange={setMcLtOvStates} error={extraErrors.mc_lt_ov_states} />
                    <FieldError msg={extraErrors.mc_lt_ov_states} />
                  </div>
                  <div>
                    <Label htmlFor="mc_lt_ov_startDate" required>Preferred Start Date</Label>
                    {dateInput('mc_lt_ov_startDate', 'input-mc-lt-ov-start-date', 'mc_lt_ov_startDate', true)}
                  </div>
                  <div>
                    <Label htmlFor="mc_lt_ov_avail" required>Availability</Label>
                    <AvailabilityPicker id="mc_lt_ov_avail" days={mcLtOvDays} onDaysChange={setMcLtOvDays}
                      hours={''} onHoursChange={() => {}} error={extraErrors.mc_lt_ov_days} />
                    <FieldError msg={extraErrors.mc_lt_ov_days} />
                  </div>
                </>}
              >
                <div>
                  <Label htmlFor="mc_lt_package">Package</Label>
                  <div id="mc_lt_package" data-testid="display-mc-lt-package" className="rounded-[10px] border border-[#243e62] bg-[#080f1c] px-3.5 py-2.5 text-sm text-[#7a95ba]">
                    {getPackageLabel(cart, 'Medicare', 'Live Transfers')}
                  </div>
                </div>
                <div>
                  <Label htmlFor="mc_lt_leadType" required>Lead Type</Label>
                  <RadioGroup name="mc_lt_leadType" options={['Supplement','Advantage','Both']}
                    register={register as (n: string, o?: object) => object}
                    error={errors.mc_lt_leadType?.message} />
                </div>
                <Row2>
                  <div>
                  ...
                  <div>
                    ...
                    <input id="mc_lt_callsPerDay" type="number" min="1" data-testid="input-mc-lt-calls-per-day" placeholder="e.g. 10"
                      className={errors.mc_lt_callsPerDay ? errorInputClass : inputClass}
                      {...register('mc_lt_callsPerDay', { required: 'Calls per day is required.', min: { value: 1, message: 'Min 1.' } })} />
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
              ...
              <textarea id="additionalInstructions" rows={4} data-testid="input-additional-instructions"
            ...
            <div className="glass-panel rounded-[20px] p-6 sm:p-7">
              ...
              <p className="mt-3 text-center text-[11px] text-[#5a7999]">No payment is collected here. Your order is reviewed by the LeadStream team.</p>
            </div>
          </form>
